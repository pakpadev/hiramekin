...
- Windows: `.exe` / `.msi`
- macOS: Apple Silicon向け `.dmg`
- 自動更新用: `.app.tar.gz` と `.sig` は今回のArtifactsでは未生成
...
https://github.com/pakpadev/hiramekin/releases/download/desktop-v1.0.9/Hiramekin_1.0.9_aarch64.dmg
```
...
1. Artifacts内のインストーラーをRelease assetsとしてアップロードする。
...
- 今回のArtifactsには `.app.tar.gz` と `.sig` が含まれていない。
- 不正なURLや署名を入れるとアップデーターが失敗するため、署名付き更新assetが出るまでは `platforms: {}` のままにする。

## 注意事項

- macOSではネイティブウィンドウの完全透明化を使っていないため、オーバーレイの透過はReact UI側の表現になる。
- 現在のmacOS配布はApple Silicon向けDMGのみ。
