# 閃筋 (Hiramekin) 設計仕様書

**作成日**: 2026-05-21  
**バージョン**: 1.5  
**ステータス**: 承認済み

---

## 概要

閃筋（ひらめきん）は、一瞬のひらめきやアイディアを瞬時に書き留めることができる殴り書きアプリ。簡単な計算も同一画面で行える。アイデアの記録と簡単な計算を「最短最速」で行うことを第一原則とする。

---

## プロダクトビジョン

**第一原則**: アプリを開いたら即座に入力できる状態にある

**v2 最重要機能**: Androidホーム画面ウィジェット（0タップで入力）  
ウィジェット対応はv1設計段階から構造を考慮する。後付けが困難なため、bare workflowを採用する理由の一つ。

---

## 対応プラットフォーム

| プラットフォーム | 提供形式 |
| --- | --- |
| Android | ネイティブアプリ |
| iOS | ネイティブアプリ |
| Windows | PWA（ブラウザインストール型） |
| macOS | PWA（ブラウザインストール型） |
| Linux | PWA（ブラウザインストール型） |

---

## 技術スタック

| 項目 | 採用技術 | 理由 |
| --- | --- | --- |
| フレームワーク | Expo bare workflow (React Native) | ウィジェット(v2)対応のためbareを選択 |
| Web/PWA | react-native-web | 単一コードベース |
| ストレージ (モバイル) | expo-sqlite または AsyncStorage | オフライン完結 |
| ストレージ (Web) | IndexedDB (idb) | オフライン完結 |
| 計算エンジン | mathjs (軽量バンドル) | 数式評価の信頼性 |
| 音声入力 (Web/PWA) | Web Speech API | ブラウザ標準の音声認識を利用 |
| 音声入力 (Android/iOS) | expo-speech-recognition | OSネイティブ音声認識をExpo/React Nativeから利用 |
| 将来の同期 | Supabase | スキーマ定義のみ、v1では未実装 |
| ナビゲーション | なし（単一画面・状態管理のみ） | 画面遷移ゼロで最速UXを実現 |

---

## アーキテクチャ

```text
[単一コードベース]
  Expo bare workflow (React Native + react-native-web)
         |
   +-----------+----------+
   |           |          |
Android      iOS    Web (PWA)
                  Windows/Mac/Linux

[レイヤー構成]
  App.tsx         単一画面エントリ（Navigator不要）
  components/
    InputArea     入力エリア（新規・編集モード共用）
    MemoList      ピン留め＋時系列リスト
    SearchBar     検索バー
    CalcOverlay      数式結果オーバーレイ
    KeyboardToolbar  キーボード上部ツールバー（横スクロール対応: タイム・=計算・議事録・マイクボタン）
    VoiceInput       音声入力オーバーレイ
  services/
    storage.ts    IStorage インターフェース定義
    adapters/
      asyncStorage.ts   モバイル実装
      indexedDB.ts      Web実装
      supabase.ts       将来用（インターフェースのみ、実装空）
    calculator.ts   数式評価ロジック
  hooks/
    useMemos      メモCRUD・検索・アーカイブ
    useCalc       数式評価
    useVoice      音声入力（Web Speech API / expo-speech-recognition）
    useHaptics    ハプティクスフィードバック（expo-haptics）
  native/         v2ウィジェット用ネイティブコード置き場（空フォルダ）
```

---

## データモデル

```typescript
interface Memo {
  id: string          // UUID
  content: string     // テキスト本文（数式込み）
  isPinned: boolean   // ピン留め
  isArchived: boolean // アーカイブ（通常リストから非表示）
  notifyAt: number | null // 通知予定時刻 (Unix timestamp ms)。null = 通知なし
  createdAt: number   // Unix timestamp (ms)
  updatedAt: number   // Unix timestamp (ms)
}

interface AppSettings {
  theme: 'system' | 'light' | 'dark' // v1は 'system' 固定
}

// 画面状態（永続化しない）
interface UIState {
  editingMemoId: string | null // null = 新規作成モード
  searchQuery: string
  showArchive: boolean         // アーカイブ一覧表示中
  isListening: boolean         // 音声入力中
}
```

### Supabase スキーマ（v1では使用しない、定義のみ）

```sql
create table memos (
  id uuid primary key default gen_random_uuid(),
  user_id uuid references auth.users,
  content text not null,
  is_pinned boolean default false,
  is_archived boolean default false,
  notify_at timestamptz default null,
  created_at timestamptz default now(),
  updated_at timestamptz default now()
);
```

---

## 画面構成（単一画面・状態ベース）

画面遷移なし。`editingMemoId` と `searchQuery` の状態によってUIが切り替わる。

### 状態: browsing（初期状態・起動時）

```text
+---------------------------+
| 新規メモ入力エリア         |  起動時カーソル自動フォーカス
| (1行分、入力で拡張)        |
+---------------------------+
| [検索バー]                |
+---------------------------+
| [ピン留め]                |
|  - メモA        15:32     |
|  - メモB        昨日      |
+---------------------------+
| [最新順]                  |
|  - メモC        14:00     |
|  - メモD        5/20      |
+---------------------------+
                    [+]       新規メモ開始（入力エリアにフォーカス）
```

### 状態: editing（メモをタップ時）

```text
+---------------------------+
| メモ内容が入力エリアに展開 |  editingMemoId がセットされる
| ふつうのテキスト           |
| 100 + 200 = [300]         |  リアルタイム計算
| 売上 1000 * 0.08 = [80]   |  [結果]タップで切替
|                           |
| [ピン留めトグル]  [通知] [アーカイブ] |
+---------------------------+
| [タイム▾][=計算][議事録][マイク] →  |  キーボード上部ツールバー（横スクロール）
+--[キーボード]--------------+
  キーボードを閉じる or 入力エリア外タップ → browsing に戻る
  （離脱時に自動保存）
```

### 状態: searching（検索バーに入力時）

```text
+---------------------------+
| 新規メモ入力エリア         |
+---------------------------+
| [検索バー: "キーワード"]   |  入力中
+---------------------------+
| 検索結果リスト             |  リアルタイムフィルタリング
|  - マッチしたメモA         |
|  - マッチしたメモB         |
+---------------------------+
  検索バークリア → browsing に戻る
```

---

## 状態遷移

```text
起動
 └─ browsing
      ├─ 入力エリアにタイプ → 新規メモ作成（editingMemoId = null のまま）
      ├─ メモをタップ → editing（editingMemoId = タップしたメモのid）
      ├─ 検索バーに入力 → searching
      └─ FABタップ → 入力エリアにフォーカス（browsing のまま）

editing
 └─ キーボードを閉じる / 入力エリア外タップ
      └─ 自動保存 → browsing

searching
 └─ 検索バークリア / キャンセル → browsing
 └─ 検索結果のメモをタップ → editing
```

---

## キーボードツールバー（クイック挿入）

キーボード展開時に上部に表示されるツールバー。カーソル位置にテキストを挿入する。  
ツールバーは**横スクロール対応**とし、ボタン数が増えても各ボタンのラベルを省略しない。

| ボタン | 短押し動作 | 長押し動作 | 用途 |
| --- | --- | --- | --- |
| タイム ▾ | `2026/05/22 14:30`（日時同時挿入） | [日付のみ] / [時刻のみ] メニュー | 日時メモの起点 |
| = 計算 | サンプル式 `合計: 100 + 200` を挿入 | なし | 計算機能の発見・体験 |
| 議事録 | 議事録テンプレートを挿入 | なし | 会議メモの起点 |
| マイク | 音声入力を開始 | なし | ハンズフリー入力 |

### 日付・時刻の編集方針

過去・未来の日付を入力する場合はカレンダーUIを使わない。「タイム」ボタンで挿入し、変えたい数字だけ手動で修正する。

```text
例: 昨日の日付だけを入力したい場合
  「タイム」長押し → [日付のみ] を選択 → 2026/05/22 が挿入される
  「22」を「21」に手動修正 → 2026/05/21
```

日付ピッカーはコンセプト（最短最速）に反するため採用しない。

### 計算機能の発見性

計算機能はキーボードツールバーで挿入するまで存在に気づきにくい。以下の2つで補助する。

- **プレースホルダー**: 入力欄が空のとき `メモを書く　　100+200 で計算できます` を表示する
- **= 計算ボタン**: 押すとサンプル式が挿入され、青文字で結果が即表示される。体験を通じて計算機能を理解できる

### 議事録テンプレート（v1 固定）

```text
## 議事録
日時: YYYY/MM/DD HH:MM
参加者: 

### 議題
- 

### 決定事項
- 

### アクションアイテム
- [ ] 担当:  期日: 
```

`日時:` の値は挿入時点の日時で自動補完する（`formatToday()` + `formatTime()` を組み合わせる）。  
テンプレートのカスタマイズは v2 以降で対応。

---

## 計算エンジン仕様

### 対応演算

| 記法 | 例 | 結果 |
| --- | --- | --- |
| 四則演算 | `100 + 200` | `300` |
| パーセント | `1000 * 0.08` | `80` |
| 小数 | `3.14 * 2` | `6.28` |
| 括弧 | `(100 + 200) * 3` | `900` |

### 非対応（v1）

- 行をまたいだ変数参照
- 関数（sin, sqrt など）

### 数式行の判定

- 行に数字と演算子（`+`, `-`, `*`, `/`, `(`, `)`）が含まれる場合のみ評価
- 評価失敗・不正数式の場合は結果欄を空白にする（エラーメッセージ表示なし）
- 結果はテキストに書き込まず、行の右隣にオーバーレイ表示する（ユーザーの入力テキストを変更しない）
- 数式行の `[結果]` 部分をタップ → 数式ごと結果のみ表示に切替（再タップで元に戻る）

---

## 音声入力

キーボードツールバーのマイクボタンで起動。`expo-speech` は Text-to-Speech 用のため、音声入力には使用しない。Web/PWAでは Web Speech API を使用し、Android/iOSでは `expo-speech-recognition` 経由で OS ネイティブ音声認識（Android: SpeechRecognizer、iOS: SFSpeechRecognizer）を呼び出す。

```text
マイクタップ
 └─ 音声入力オーバーレイ表示（isListening = true）
      ├─ 認識中: 波形アニメーション表示
      ├─ 認識テキスト確定 → カーソル位置に挿入、オーバーレイ非表示
      └─ キャンセル / タイムアウト → オーバーレイ非表示
```

- 認識結果はテキストとしてそのまま挿入（変換確認なし）
- 音声入力後も通常のテキスト編集が可能
- PWA（デスクトップ）では Web Speech API を使用
- Android/iOS では `expo-speech-recognition` を使用
- `expo-speech-recognition` はネイティブモジュールと config plugin を含むため、Android/iOSビルド前に Expo prebuild で native project へ反映する

---

## アーカイブ

削除の代わりにアーカイブへ退避する。通常リストには表示されないが失われない。

### 操作

- editingモードのヘッダー「アーカイブ」ボタンタップ → 即アーカイブ（確認ダイアログなし）
- アーカイブ一覧は SettingsScreen 内の「アーカイブを見る」からアクセス

### 自動削除ルールの変更

| ケース | 旧動作 | 新動作 |
| --- | --- | --- |
| 既存メモを空にして離脱 | 即削除 | アーカイブに移動 |
| 新規メモを空のまま離脱 | 保存しない | 保存しない（変わらず） |

### アーカイブ一覧

```text
SettingsScreen
 └─ アーカイブを見る
      ├─ アーカイブ済みメモの一覧（時系列）
      ├─ タップ → 復元（isArchived = false）
      └─ 完全削除ボタン（長押しで確認）
```

---

## ハプティクスフィードバック

`expo-haptics` を使用。モバイルのみ有効（PWAでは無視）。

| タイミング | 種類 | 意味 |
| --- | --- | --- |
| 自動保存完了 | Light Impact | 「捕まえた」の確認 |
| アーカイブ実行 | Medium Impact | 操作の区切り |
| ピン留めトグル | Selection Changed | 状態変更 |

UIアニメーションを使わずに操作の手応えを伝えるため、アニメーション「最小限」方針と矛盾しない。

---

## 通知機能

メモに日付が含まれている場合、その日時の前に通知を送ることができる。

### プラットフォーム対応

| プラットフォーム | v1 | v2 |
| --- | --- | --- |
| Android | expo-notifications | 同左 |
| iOS | expo-notifications | 同左 |
| Windows / macOS / Linux (PWA) | 非対応 | Web Push + Service Worker |

### 日付の検出

メモ内容を走査し、キーボードツールバーが挿入する `YYYY/MM/DD` 形式の最初の日付を検出する。複数の日付が含まれる場合は最初の1件のみ対象とする（v1）。

### UI フロー

```text
editingモード: メモ内に YYYY/MM/DD が検出される
 └─ ヘッダーに [通知] ボタンが出現
      └─ タップ → 事前通知時間を選択
           ├─ 当日 朝8時
           ├─ 1日前 朝8時
           ├─ 1週間前 朝8時
           └─ 通知しない（設定済みを解除）
      └─ 選択 → notifyAt を計算して保存 → 通知をスケジュール
```

### 通知のスケジューリング

- `notifyAt` が保存されると `expo-notifications` でローカル通知をスケジュールする
- メモが更新・アーカイブされた場合は既存の通知をキャンセルして再スケジュールする
- 通知タップ → アプリを開き、対象メモを editing モードで表示する

### 通知権限

- アプリ初回起動時ではなく、ユーザーが最初に通知を設定しようとしたタイミングで権限を要求する
- 権限が拒否された場合はトーストで案内する（強制しない）

---

## 自動保存ロジック

```text
新規メモ:
  入力開始と同時にIDを発行してストレージに保存
  以降は500msデバウンスで差分保存

既存メモ編集:
  テキスト変更のたびに500msデバウンスで保存
  editingモード離脱時（キーボードclose / 外タップ）に即時保存

空のメモ:
  入力エリアが空のままeditingモードを離脱した場合は保存しない（新規のみ）
  既存メモを空にして離脱した場合はアーカイブに移動（即削除しない）
```

---

## エラーハンドリング方針

| ケース | 対応 |
| --- | --- |
| 不正な数式 | 結果欄を空にして無視、クラッシュしない |
| ストレージ書き込み失敗 | メモリキャッシュで保持、次回起動時に再試行 |
| アプリ強制終了 | リアルタイム自動保存のためデータ損失なし |
| PWAオフライン | ローカルのみのためオフラインで全機能利用可 |

---

## UIデザイン方針

- テーマ: OS設定に従うダーク/ライト自動切替
- フォント: システムフォント（追加フォントなし、軽量化）
- アニメーション: 最小限（状態遷移のフェードのみ）
- ナビゲーションバー: なし
- カラー: Neutral グレーベース、アクセントカラー1色

---

## SettingsScreen（設定）

単一画面構成の例外として設定画面のみ別画面として保持する。  
アクセス頻度が極めて低いため、画面遷移コストが問題にならない。  
v1は設定項目なし（将来用に画面だけ用意）。

---

## v2 ロードマップ（本仕様の対象外）

| 優先度 | 機能 |
| --- | --- |
| 最重要 | Androidホーム画面ウィジェット |
| 高 | 通知機能のPWA/デスクトップ対応（Web Push + Service Worker） |
| 高 | Supabaseによるデバイス間同期 |
| 中 | タグ機能 |
| 中 | iOSウィジェット（Swiftが必要） |
| 低 | 行をまたいだ変数参照 |

---

## 非機能要件

- アプリ起動 → 入力可能状態まで 1秒以内
- メモ自動保存レイテンシ: 500ms 以内（デバウンス）
- 検索レスポンス: 200ms 以内（ローカル検索のため）
- 最小サポートバージョン: Android 10 / iOS 15 / 主要ブラウザ最新2世代
