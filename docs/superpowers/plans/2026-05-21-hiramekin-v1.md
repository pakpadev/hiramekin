...
**Tech Stack:** Expo bare workflow, React Native, react-native-web, TypeScript, mathjs, idb, expo-haptics, expo-speech, expo-notifications, @testing-library/react-native, jest-expo
...
```bash
npm install mathjs idb @react-native-async-storage/async-storage expo-haptics expo-speech expo-notifications react-native-web react-dom
...
- [ ] **Step 8: `app.json` にアイコン・スプラッシュ・通知権限を設定する**
...
- [ ] **Step 9: `native/` フォルダとスキーマファイルを作成する**
...
```bash
docker exec -it hiramekin-test-1 npx jest __tests__/services/notifications.test.ts --no-coverage
```
...
- [ ] **Step 5: useNotifications フックを実装する**
...
- [ ] **Step 6: NotifyPicker コンポーネントを実装する**
